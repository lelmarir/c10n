...
public abstract class C10NConfigBase {
	...
	/**
	 * <p>
	 * To be implemented by subclasses of {@link C10NConfigBase}.
	 *
	 * <p>
	 * Configuration methods are as follows:
	 * <ul>
	 * <li>{@link #bindAnnotation(Class)} - binds annotation that holds translation
	 * for a specific locale.</li>
	 * <li>{@link #bindBundle(String)} - binds a resource bundle containing
	 * translated messages.</li>
	 * <li>{@link #install(C10NConfigBase)} - includes configuration from another
	 * c10n configuration module</li>
	 * <li>{@link #bind(Class)} - binds a custom class as an implementation for the
	 * given c10n interface</li>
	 * <li>{@link #setLocaleProvider(LocaleProvider)} - customises the locale
	 * retrieval logic</li>
	 * <li>{@link #setUntranslatedMessageHandler(UntranslatedMessageHandler)} -
	 * customises the placeholder for unresolved translation mappings</li>
	 * <li>{@link #setKeyPrefix(String)} - sets global key prefix to auto-prepend to
	 * all bundle keys</li>
	 * </ul>
	 */
	 ...
	 *
	 * <p>
	 * The name of the package is used to determine which c10n interfaces this
	 * configuration is responsible for
	 *
	 * <p>
	 * The default implementation, which returns the string representation of the
	 * package of the configuration class, should suffice in most cases
	 *
	 ...
	 *
	 * <p>
	 * This will apply the configuration to all c10n interfaces located in the child
	 * configuration package or below.
	 *
	 ...
	 * </pre>
	 *
	 * which will use the <code>JapaneseMessagesImpl.class</code> when locale is set
	 * to <code>Locale.JAPANESE</code>
	 *
	 * <p>
	 * The second usage is:
	 * 
	 ...
	 *
	 * @param c10nInterface C10N interface to create an implementation binding for
	 *                      (not-null)
	...
	/**
	 * <p>
	 * Sets the {@link LocaleProvider} for this configuration.
	 *
	 * <p>
	 * Locale provider is consulted every time a translation is requested, that is
	 * every time a method on an c10n interface is called.
	 *
	 * <p>
	 * As a rule, there should be only one locale provider per application. Any
	 * locale providers defined in child configurations (see
	 * {@link #install(C10NConfigBase)} are disregarded.
	 *
	 * <p>
	 * Because locale provider has to be consulted on every translation request
	 * {@link com.github.rodionmoiseev.c10n.LocaleProvider#getLocale()} should avoid
	...
	/**
	 * <p>
	 * Fixes the {@link java.util.Locale} to the specified locale.
	 *
	 * <p>
	 ...
	 * </pre>
	 *
	 * @param handler custom implementation of untranslated message handler
	 *                (not-null)
	 */
	...
	/**
	 * <p>
	 * Overrides the classloader used for loading c10n-interface proxies. This maybe
	 * useful in the context of hot-swapping enabled classloaders, like the one for
	 * Play framework 2.0, or OSGi.
	 *
	 * <p>
	 * By default, the classloader of
	 * {@link com.github.rodionmoiseev.c10n.C10N#getClass()} class is used.
	 *
	 * @param proxyClassLoader the classloader to use for loading c10n-interface
	 *                         proxies (not-null)
	 */
	...
	/**
	 * <p>
	 * The c10n intefrace proxy classloader that will be used by the current
	 * instance of c10n message factory.
	 * </p>
	 *
	 * @return the classloader to be used for loading c10n-interface proxies
	 *         (not-null)
	 */
	 ...
	 * </pre>
	 *
	 * which will make c10n always fallback to the value given in the
	 * <code>@En</code> annotation if no other annotation binding matched the
	 * current locale.
	 *
	 * <p>
	 * Note: Some default annotation bindings are defined in
	 * {@link com.github.rodionmoiseev.c10n.annotations.DefaultC10NAnnotations}. In
	 * order to use <code>install(new DefaultC10NAnnotations());</code> somewhere in
	 * your configuration (see {@link #install(C10NConfigBase)}
	 *
	 * @param annotationClass Class of the annotation to create a local binding for
	 *                        (not-null)
	 ...
	 * <p>
	 * All arguments passed to c10n-interfaces with the specified type(s) will be
	 * converted to string using the filter generated by the given filter provider,
	 * instead of the conventional <code>toString()</code> method.
	 *
	 * <p>
	 * Filter creation (using
	 * {@link com.github.rodionmoiseev.c10n.C10NFilterProvider#get()} method) will
	 * be deferred until the first call to a c10n-interface method with a matching
	 * argument type is executed.
	 *
	 * @param c10NFilterProvider provider of filter implementation (not-null)
	 * @param type               method argument type to which the filter should be
	 *                           applied
	 * @param <T>                method argument type to which the filter should be
	 *                           applied
	 ...
	 * <p>
	 * All arguments passed to c10n-interfaces with the specified type(s) will be
	 * converted to string using this filter, instead of the conventional
	...
	/**
	 * <p>
	 * Set global key prefix. All other keys will be automatically prepended with
	 * the global key.
	 * <p>
	 * Settings key prefix to an empty string resets to default behaviour (no
	 * prefix).
	 *
	...
	/**
	 * <p>
	 * If set to 'true', c10n will output debugging information to std-out at
	 * configuration and lookup time.
	 *
	...
	private void checkAnnotationInterface(Class<? extends Annotation> annotationClass) {
		if (noMethod(annotationClass, "value") && noMethod(annotationClass, "intRes")
			...
			throw new C10NConfigException("Annotation could not be bound because it's missing any of value(),"
					...
					+ "Please add at least one of those methods with return type of String. " + "annotationClass="
	...
	List<ResourceBundle> getBundlesForLocale(Class<?> c10nInterface, Locale locale) {
		...
		for (Entry<String, C10NBundleBinder> entry : bundleBinders.entrySet()) {
			...
			if (binder.getBoundInterfaces().isEmpty() || binder.getBoundInterfaces().contains(c10nInterface)) {
				try {
	...
	/**
	 * For each annotation bound in this configuration find all locales it has been
	 * bound to.
	 *
	...
	/**
	 * Find all locales that have explicit implementation class bindings for this
	 * c10n interface.
	 *
	 ...
	 * <p>
	 * Use {@link #annotatedWith(Class)} method to restrict the filter to arguments
	 * annotated with the specified annotation(s). Multiple annotations may be
	 * specified using chained {@link #annotatedWith(Class)} methods.
	 */
	protected static final class C10NFilterBinder<T> {
		...
		/**
		 * <p>
		 * Restrict filter application only to arguments annotated with the given
		 * annotation.
		 * <p>
