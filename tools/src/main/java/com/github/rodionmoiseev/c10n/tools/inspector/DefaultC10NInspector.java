...
class DefaultC10NInspector implements C10NInspector {
	...
	DefaultC10NInspector(C10NInterfaceSearch c10NInterfaceSearch, ConfiguredC10NModule configuredC10NModule,
			DummyInstanceProvider dummyInstanceProvider, Set<Locale> localesToCheck, boolean fetchTranslations) {
	...
	public List<C10NUnit> inspect(String... packagePrefixes) {
		...
		for (Class<?> c10nInterface : c10nInterfaces) {
			Set<Map.Entry<Class<? extends Annotation>, Set<Locale>>> annotationEntries = configuredC10NModule
				...
				for (Map.Entry<Class<? extends Annotation>, Set<Locale>> entry : annotationEntries) {
					...
					for (Locale locale : entry.getValue()) {
						if (localesToCheck.contains(locale)) {
							String translatedValue = extractTranslatedValue(c10NMsgFactory, c10nInterface, method,
									locale);
			...
			for (Locale locale : localesToCheck) {
				...
				for (C10NUnit unit : unitsForInterface) {
					String translatedValue = extractTranslatedValue(c10NMsgFactory, c10nInterface,
							unit.getDeclaringMethod(), locale);
					C10NTranslations trs = addTranslations(unit, locale, translatedValue);
	...
	private String extractTranslatedValue(C10NMsgFactory c10NMsgFactory, Class<?> c10nInterface, Method method,
			Locale locale) {
		if (fetchTranslations) {
			...
			for (int i = 0; i < args.length; i++) {
				...
				if (null == args[i]) {
					throw new C10NInspectorException(
							"Cannot create dummy instance for" + "type: " + paramTypes[i].getName());
				}
			}
			Object v;
			try {
				v = method.invoke(c10NMsgFactory.get(c10nInterface, locale), args);
	...
	private C10NUnit addC10NUnit(Map<C10NBundleKey, C10NUnit> unitByKey, Class<?> declaringInterface,
			Method declaringMethod, C10NBundleKey key) {
	...
	private static final class C10NInspectorException extends RuntimeException {
